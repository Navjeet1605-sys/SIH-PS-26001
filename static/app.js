...
async function drawEvacuationRoute(locId) {
  try {
    ...
    // Use real road network geometry (following actual national highways & mountain lanes)
    const latlngs = (route.real_road_coords && route.real_road_coords.length > 0)
      ? route.real_road_coords
    ...
    // Auto-fit map viewport to display the entire curved road corridor cleanly
    try {
    ...
    if (info) {
      const gmapsBtn = route.google_maps_url ? `
        <a href="${route.google_maps_url}" target="_blank" rel="noopener noreferrer" 
           class="brutal-btn brutal-btn-orange" 
           style="display:inline-flex; align-items:center; gap:6px; margin-top:8px; text-decoration:none; width:100%; justify-content:center; font-size:0.8rem; padding:8px 10px; font-weight:900;">
          🗺️ Launch Google Maps Direct Navigation (Real Lanes)
        </a>
      ` : "";
      
      const isReal = route.is_real_road_geometry ? "✓ Real Highway Lanes (OSRM & Google Maps)" : "✓ Hazard Bypass Corridor";

      info.innerHTML = `
        <div style="background:var(--bg-card); border:1.5px solid #000; border-radius:6px; padding:10px; box-shadow:2px 2px 0 #000;">
          <div style="font-weight:900; color:var(--text-main); margin-bottom:4px; font-size:0.85rem;">🏥 Nearest Safe Zone: ${route.to_safe_zone.name}</div>
          <div style="font-size:0.8rem;">📏 Real Road Distance: <b>${route.estimated_distance_km} km</b> · ⏱️ Highway ETA: <b>${route.estimated_time_min} mins</b></div>
          <div style="font-size:0.72rem; color:var(--green-dark); font-weight:800; margin-top:4px;">${isReal}</div>
          ${gmapsBtn}
        </div>
      `;
    }
...
async function submitReport() {
  ...
  try {
    ...
    const isVer = res.is_verified || res.verification_status === "VERIFIED_REAL";
    const verBadge = isVer 
      ? `<span style="background:#dcfce7; color:#15803d; padding:2px 6px; border-radius:4px; font-weight:800; font-size:10px; border:1px solid #15803d;">✓ VERIFIED REAL (${(res.verification_score * 100).toFixed(0)}%)</span>`
      : `<span style="background:#fee2e2; color:#b91c1c; padding:2px 6px; border-radius:4px; font-weight:800; font-size:10px; border:1px solid #b91c1c;">⚠️ UNVERIFIED / SPAM (${(res.verification_score * 100).toFixed(0)}%)</span>`;

    const marker = L.marker([body.lat, body.lng], {
      icon: L.divIcon({
        className: "",
        html: `<div style="background:${isVer ? '#10b981' : '#ef4444'}; border:2px solid #000; border-radius:50%; width:26px; height:26px; display:flex; align-items:center; justify-content:center; box-shadow:2px 2px 0 #000; font-size:14px;">📍</div>`,
        iconSize: [26, 26],
        iconAnchor: [13, 13]
      })
    }).addTo(map).bindPopup(`
      <div style="font-family:'Plus Jakarta Sans'; font-size:12px; max-width:220px;">
        <div style="margin-bottom:4px;">${verBadge}</div>
        <b style="color:#c2410c;">📸 CITIZEN INCIDENT REPORT</b><br>
        <b>Severity:</b> ${body.reported_severity}<br>
        <b>Details:</b> ${body.description}<br>
        <div style="font-size:10px; color:#64748b; margin-top:4px;">Stored in <code>reports.db</code></div>
      </div>
    `);
    ...
    showToast(`Report logged to reports.db! Status: ${res.verification_status} (${(res.verification_score * 100).toFixed(0)}%)`, isVer ? "success" : "error");
  } catch (err) {
...
// ---------------------------------------------------------------------------
// Services Hub Menu Toggle & Hamburger Icon Switcher
// ---------------------------------------------------------------------------
function toggleServicesMenu() {
  ...
  const icon = document.getElementById("servicesBtnIcon");
  const text = document.getElementById("servicesBtnText");
  ...
  if (isShowing) {
    ...
    if (text) text.textContent = "All Services";
  } else {
    menu.classList.add("show");
    if (backdrop) backdrop.classList.add("show");
    if (btn) btn.classList.add("active");
    if (icon) icon.textContent = "✕";
    if (text) text.textContent = "Close Menu";
  }
}
